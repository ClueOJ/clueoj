...
class ExternalProblem(models.Model):
    ...
    language_mappings = models.JSONField(
        default=list,
        blank=True,
        verbose_name=_('language mappings'),
        help_text=_('List of mappings with clueoj_key, vjudge_id, and vjudge_name.'),
    )
    ...
    def get_language_mapping(self, language):
        ...
        for mapping in self.language_mappings or []:
            if str(mapping.get('clueoj_key') or '').strip().lower() == key:
                return mapping
        return None

    def get_vjudge_language(self, language):
        mapping = self.get_language_mapping(language)
        if not mapping:
            return None
