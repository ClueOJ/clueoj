...
SELECTED_LANGUAGE_RESPONSE_KEY = '_clueoj_selected_external_language'
...
def raw_response_with_selected_language(raw_response, mapping):
    ...
    if mapping_vjudge_id(mapping):
        data[SELECTED_LANGUAGE_RESPONSE_KEY] = {
            'clueoj_key': mapping.get('clueoj_key') or '',
...
def schedule_external_submission(submission, rejudge=False, batch_rejudge=False):
    ...
    try:
        ...
        set_external_submission_error(submission, _('This problem is not configured for external judge.'))
        return False

    if not external_problem.is_active:
        set_external_submission_error(submission, _('This problem is temporarily not accepting submissions.'))
        return False
    ...
    if not vjudge_language_id:
        set_external_submission_error(
            submission,
            _('Language %(language)s is not supported for this problem.') % {
    ...
    ExternalSubmission.objects.update_or_create(
        ...
        defaults={
            ...
            'pcd_submission_id': expected_submission_id,
    ...
    try:
        submit_external_submission.apply_async(
            args=[submission.id, rejudge, batch_rejudge, str(expected_submission_id)],
        )
    except Exception:
        logger.exception(
        ...
        set_external_submission_error(
            submission,
            _('Could not enqueue the external judge submission. Please try again later.'),
        )
        return False
...
def perform_external_submission(submission, rejudge=False, batch_rejudge=False, expected_submission_id=None):
    submission.refresh_from_db()
    ...
    if expected_submission_id is not None:
        ext_sub_filter['pcd_submission_id'] = expected_submission_id

    if not ExternalSubmission.objects.filter(**ext_sub_filter).exists():
        logger.info(
            'Ignoring stale external submit task: submission_id=%s expected_submission_id=%s',
            submission.id,
    ...
    external_submission_id = response.get('submissionId')
    system_status = response.get('systemStatus') or response.get('status') or 'pending'
    if external_result_is_submit_failed(response) or not external_submission_id:
        ...
        error_msg = external_error_message(response, _('External judge rejected this submission.'))
        set_external_submission_error(submission, error_msg)
        return False

    try:
        external_submission_id = uuid.UUID(str(external_submission_id))
    except (TypeError, ValueError):
        set_external_submission_error(submission, _('External judge returned an invalid submissionId.'))
        return False

    update_filter = {'id': ext_sub.id}
    if expected_submission_id is not None:
        update_filter['pcd_submission_id'] = expected_submission_id

    updated = ExternalSubmission.objects.filter(**update_filter).update(
        config=external_problem.config,
        pcd_submission_id=external_submission_id,
    ...
    if not updated:
        logger.info(
            'Ignoring stale external submit result: submission_id=%s expected_submission_id=%s actual=%s',
            submission.id,
            expected_submission_id,
    ...
    poll_external_submission.apply_async(
        args=[submission.id, str(external_submission_id)],
...
def finalize_external_submission(submission, ext_sub, data):
    ...
    if external_result_is_submit_failed(data):
        status, result = 'IE', 'IE'
        error_message = error_message or _('External judge rejected this submission.')
        ...
        status, result = 'IE', 'IE'
        error_message = _('External judge returned an unsupported status.')
