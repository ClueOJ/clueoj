...
class VOIContestFormat(DefaultContestFormat):
    ...
    def update_participation(self, participation):
        ...
        score = 0
        format_data = {}
        counted_submission = self.config['counted_submission']
        ...
        queryset = (participation.submissions.values('problem_id')
                                     ...
                                     .annotate(time=Min('submission__date'), points=Subquery(
                                         participation.submissions.filter(problem_id=OuterRef('problem_id'))
                                                                  .order_by('submission__date')
                                                                  .values('points')[:1]))
    ...
    def get_short_form_display(self):
        yield _('The first submission for each problem will be used.')
