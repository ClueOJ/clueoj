...
class ExternalJudgeError(Exception):
    user_message = _('External judge is unavailable. Please try again later.')
...
class ExternalJudgeAuthError(ExternalJudgeError):
    user_message = _('External judge configuration error. Please contact an administrator.')
...
class ExternalJudgeUpstreamError(ExternalJudgeError):
    user_message = _('The external OJ is unavailable.')


class ExternalJudgeNotFoundError(ExternalJudgeError):
    user_message = _('External judge problem was not found.')


class ExternalJudgeBadRequestError(ExternalJudgeError):
    user_message = _('External judge rejected this request.')
...
class ExternalJudgeClient:
    ...
    def _request(self, method, path, *, timeout=None, **kwargs):
        ...
        source_present = isinstance(request_kwargs.get('json'), dict) and 'source' in request_kwargs['json']
        logger.info(
