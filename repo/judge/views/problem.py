...
class ProblemSubmit(LoginRequiredMixin, ProblemMixin, TitleMixin, SingleObjectFormView):
    ...
    def has_external_problem(self):
        return bool(getattr(self.object, 'has_external_problem', False))

    def get_external_problem(self):
        if not self.has_external_problem():
            return None
        return getattr(self.object, 'external_problem', None)

    def get_external_language_mappings(self):
        external_problem = self.get_external_problem()
        if external_problem is None:
            return []
        try:
            mappings = external_problem.language_mappings or []
        except Exception:
            return []
        return [
            mapping for mapping in mappings
            if mapping.get('vjudge_id')
        ]
    ...
    def get_external_mapping_language(self, mapping):
        language = self._language_for_keys([mapping.get('clueoj_key')])
        if language is not None:
            return language
    ...
    def get_external_language_options(self):
        ...
        for index, mapping in enumerate(self.get_external_language_mappings()):
            ...
            vjudge_id = mapping.get('vjudge_id')
            vjudge_name = mapping.get('vjudge_name') or vjudge_id
            clueoj_key = mapping.get('clueoj_key') or vjudge_name
            options.append({
                'token': str(index),
                'mapping': mapping,
                'language': language,
    ...
    def get_default_external_language_token(self):
        ...
        for option in options:
            clueoj_key = option['mapping'].get('clueoj_key')
            if clueoj_key and clueoj_key.lower() == self.default_language.key.lower():
                return option['token']
    ...
    def get_form_kwargs(self):
        ...
        if self.has_external_problem() and kwargs.get('data') is not None:
    ...
    def form_valid(self, form):
        ...
        if self.has_external_problem():
            if not getattr(self.object, 'is_external', False):
                return generic_message(
                    self.request,
                    _('External judge unavailable'),
                    _('This problem is temporarily not accepting submissions.'),
                    status=503,
                )
            ...
            if external_mapping is None:
                return generic_message(
                    ...
                    _('The selected language is not supported for this problem.'),
                    status=400,
                )
        else:
            external_mapping = None
        if (
            not self.has_external_problem() and
    ...
    def get_context_data(self, **kwargs):
        ...
        context['external_problem'] = self.get_external_problem()
        context['can_view_external_meta'] = bool(
            self.has_external_problem() and
...
class ProblemCreate(PermissionRequiredMixin, TitleMixin, CreateView):
    ...
    def get_initial(self):
        ...
        initial['memory_limit'] = 1048576  # 1 GB
        initial['partial'] = True
        initial['points'] = 10
        initial['batch_type'] = 'sum'
        try:
