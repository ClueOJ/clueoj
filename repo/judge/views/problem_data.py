...
from judge.models import Language, Problem, ProblemData, ProblemTestCase, Submission, problem_data_storage
...
try:
    from judge.models import ExternalJudgeConfig, ExternalProblem
    from judge.utils.external_judge_client import ExternalJudgeClient, ExternalJudgeError, user_safe_message
except (ImportError, AttributeError):
    ExternalJudgeConfig = None
    ExternalProblem = None
    ExternalJudgeClient = None
    ExternalJudgeError = Exception

    def user_safe_message(exc):
        return str(exc)
...
def _problem_external_problem(problem):
    if problem is None or ExternalProblem is None:
        return None
    try:
        return problem.external_problem
    except ExternalProblem.DoesNotExist:
        return None


def _problem_has_external_problem(problem):
    external_problem = _problem_external_problem(problem)
    return bool(external_problem and external_problem.is_active)


def _problem_is_external(problem):
    external_problem = _problem_external_problem(problem)
    return bool(external_problem and external_problem.is_available)
...
if ExternalProblem is not None:
    class ExternalJudgeForm(ModelForm):
        ...
        def __init__(self, *args, **kwargs):
            ...
            if self.instance.pk:
                ...
                self.initial['language_mappings'] = json.dumps(
                    self.instance.language_mappings or [], ensure_ascii=False,
                )
        ...
        def clean(self):
            ...
            for mapping in mappings:
                if not isinstance(mapping, dict):
                    continue
                clueoj_key = (mapping.get('clueoj_key') or '').strip()
                vjudge_id = str(mapping.get('vjudge_id') or '').strip()
                vjudge_name = (mapping.get('vjudge_name') or '').strip()
                if clueoj_key and vjudge_id:
                    canonical_clueoj_key = language_keys.get(clueoj_key.lower())
                    if enabled and canonical_clueoj_key is None:
                        invalid_site_keys.append(clueoj_key)
                        continue
                    normalized.append({
                        'clueoj_key': canonical_clueoj_key or clueoj_key,
                        'vjudge_id': vjudge_id,
            ...
            if invalid_site_keys:
                self.add_error(
                    'language_mappings',
                    _('Unknown ClueOJ language key(s): %(keys)s. Use an existing language key.') % {
                        'keys': ', '.join(sorted(set(invalid_site_keys))),
                    },
                )
            if enabled and not normalized and not invalid_site_keys:
    ...
    class ExternalJudgeForm(Form):
        enabled = BooleanField(required=False, label=gettext_lazy('Enable Virtual Judge'))
        config = ChoiceField(choices=(), required=False, label=gettext_lazy('VJudge server'))
        oj = CharField(required=False, label=gettext_lazy('OJ identifier'))
        external_problem_id = CharField(required=False, label=gettext_lazy('external problem ID'))
        language_mappings = CharField(required=False, widget=Textarea(attrs={'style': 'display: none;'}))
        metadata_cache = CharField(required=False, widget=HiddenInput)

        def __init__(self, *args, **kwargs):
            kwargs.pop('problem', None)
            kwargs.pop('enabled_override', None)
        ...
        def clean(self):
            ...
            if cleaned_data.get('enabled'):
                raise ValidationError(_('Virtual Judge is not configured on this site.'))
            return cleaned_data
...
class ProblemMirrorForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.is_bound:
            if _problem_has_external_problem(target_problem):
    ...
    def clean(self):
        cleaned_data = super(ProblemMirrorForm, self).clean()
...
class ProblemDataView(TitleMixin, ProblemManagerMixin):
    ...
    def get_external_form(self, post=False, enabled_override=None):
        instance = None
        if ExternalProblem is not None:
            instance = _problem_external_problem(self.object) or ExternalProblem(problem=self.object)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['is_external_problem'] = _problem_has_external_problem(self.object)
        context['external_problem'] = _problem_external_problem(self.object)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.check_valid(mirror_form, external_form, data_form, cases_formset):
            ...
            if ExternalProblem is not None and (external_form.cleaned_data.get('enabled') or external_form.instance.pk):
...
def problem_data_file(request, problem, path):
    ...
    # If direct download is enabled, redirect to an R2 presigned URL.
    # This works even if the local archive has been evicted: the canonical
    # zipfile plus every other manifest file (checker, grader, init.yml, ...)
    # can be served straight from R2.
    if getattr(settings, 'STORAGE_DIRECT_DOWNLOAD_ENABLED', False):
        ...
        is_canonical_zip = bool(data and data.zipfile and data.zipfile.name == os.path.join(problem, path))
        if ready and (is_canonical_zip or not problem_data_storage.exists(os.path.join(problem, path))):
            download = request_download_url(str(object.pk), path=None if is_canonical_zip else path)
...
def search_external_problems(request, problem):
    ...
    if ExternalJudgeConfig is None or ExternalJudgeClient is None:
        return JsonResponse({'ok': False, 'error': _('Virtual Judge is not configured on this site.')}, status=400)
...
def verify_external_problem(request, problem):
    ...
    site_languages = list(Language.objects.filter(include_in_problem=True).order_by('name', 'key'))
    suggestions = []
    for entry in languages:
        ...
        suggestions.append({
            ...
            'name': vjudge_name,
            'suggested_clueoj_key': _suggest_language_key(vjudge_name, site_languages),
        })
